...
const actions: Action[] = [
    ...
    { type: 'inbound', key: 4 },
    { type: 'inbound', key: 5 },
    { type: 'typing', key: 6, word: 'I could try to add a cute sticker on it. Do you like this?' },
    { type: 'outbound', key: 6 },
    { type: 'inbound', key: 7 },
...
export default function () {
    ...
    return (
        <>
            ...
            {isDraged && step < 12 && <div className={styles.orderPaidInput} onClick={() => setIsDraged(false)} ></div>}

            <ChannelBar />
            
            <AutomaticTyping
                width={666} height={85} left={197} top={736}
                sendWidth={70} sendHeight={24} sendLeft={769} sendTop={835}
                onInBound={(action: Action) => { setStep(Number(action.key) + 1) }} onOutBound={(action: Action) => { setStep(Number(action.key) + 1) }}
                actions={actions} />
        </>
    );
}
